...
回答一覧の「アンケートPDF出力・印刷」で作成した用紙は、「画像・PDFから回答を登録」から読み取れます。スマートフォンで真上から撮影したJPEG・PNG・WebP画像、またはスキャナーで保存した画像・PDFを使用できます。PDFはブラウザ内でページごとの画像へ変換されます。1件分の全ページを追加すると、用紙を自動識別し、単一選択・複数選択・表形式の回答を入力画面へ反映します。
